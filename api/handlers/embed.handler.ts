...
export class EmbeddingHandler implements IRequestHandler<ICreateEmbeddingRequestDTO, Result<any>> {
  async handle(request: ICreateEmbeddingRequestDTO): Promise<Result<any>> {
    ...
    const documentEmbedding = await embeddingRepository.createDocumentsAndEmbeddings(title, documentType, domain);
