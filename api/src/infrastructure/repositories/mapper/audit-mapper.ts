...
export class CartItemMapper implements IMapper<CartItem, CartItemDataModel> {
      ...
      const cartItemDocument: CartItemDataModel = {
        ...
        selectedItems: selectedItems?.length
          ? selectedItems.map((item) =>
              this.selectedCartItemMapper.toPersistence(item),
            )
          : [],
        auditCreatedBy,
        auditCreatedDateTime,
  ...
  toDomain(model: CartItemDataModel): CartItem {
    ...
    if (selectedItems?.length) {
      selectedItemsToDomain = selectedItems.map((item) =>
        this.selectedCartItemMapper.toDomain(item),
      );
    }
    const entity: CartItem = CartItem.create(
      {
        ...
        orderId,
        total,
        selectedItems: selectedItemsToDomain,
